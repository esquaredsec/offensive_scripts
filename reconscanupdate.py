...
def write_to_file(ip_address, enum_type, data):
    ...
    for path in paths:
        if enum_type == "portscan":
            subprocess.check_output("sed INSERTTCPSCAN \"" + data + "\"  -- " + path, shell=True)
        if enum_type == "dirb":
            subprocess.check_output("sed INSERTDIRBSCAN \"" + data + "\"  -- " + path, shell=True)
        if enum_type == "nikto":
            subprocess.check_output("sed INSERTNIKTOSCAN \"" + data + "\"  -- " + path, shell=True)
        if enum_type == "ftp-connect":
            subprocess.check_output("sed INSERTFTPTEST \"" + data + "\"  -- " + path, shell=True)
        if enum_type == "smtp-connect":
            subprocess.check_output("sed INSERTSMTPCONNECT \"" + data + "\"  -- " + path, shell=True)
        if enum_type == "ssh-connect":
            subprocess.check_output("sed INSERTSSHCONNECT \"" + data + "\"  -- " + path, shell=True)
        if enum_type == "pop3-connect":
            subprocess.check_output("sed INSERTPOP3CONNECT \"" + data + "\"  -- " + path, shell=True)
        if enum_type == "curl":
            subprocess.check_output("sed INSERTCURLHEADER \"" + data + "\"  -- " + path, shell=True)
    return
...
def httpsEnum(ip_address, port):
    ...
    HTTPSCANS = "nmap -sV -Pn  -p %s --script-args=unsafe=1 --script-args=vulns.showall --script=http-vhosts,http-userdir-enum,http-apache-negotiation,http-backup-finder,http-config-backup,http-default-accounts,http-methods,http-method-tamper,http-passwd,http-robots.txt,http-devframework,http-enum,http-frontpage-login,http-git,http-iis-webdav-vuln,http-php-version,http-robots.txt,http-shellshock,http-vuln-cve2015-1635 -oN ../reports/%s/%s_http.nmap %s" % (port, ip_address, ip_address, ip_address)
...
def mssqlEnum(ip_address, port):
    ...
    MSSQLSCAN = "nmap -sV -Pn -p %s --script-args=unsafe=1 --script-args=vulns.showall --script=ms-sql-info,ms-sql-config,ms-sql-dump-hashes,mysql-empty-password,mysql-brute,mysql-users,mysql-variables,mysql-vuln-cve2012-2122 --script-args=mssql.instance-port=1433,mssql.username=sa,mssql.password=sa -oN ../reports/%s/mssql_%s.nmap %s" % (port, ip_address, ip_address)
...
def smtpEnum(ip_address, port):
    ...
    SMTPSCAN = "nmap -sV -Pn -p %s --script-args=unsafe=1 --script-args=vulns.showall --script=smtp-commands,smtp-enum-users,smtp-vuln-cve2010-4344,smtp-vuln-cve2011-1720,smtp-vuln-cve2011-1764 %s -oN ../reports/%s/smtp_%s.nmap" % (port, ip_address, ip_address, ip_address)
...
def ftpEnum(ip_address, port):
    ...
    FTPSCAN = "nmap -sV -Pn -p %s --script-args=unsafe=1 --script-args=vulns.showall --script=ftp-anon,ftp-bounce,ftp-libopie,ftp-proftpd-backdoor,ftp-vsftpd-backdoor,ftp-vuln-cve2010-4221 -oN '../reports/%s/ftp_%s.nmap' %s" % (port, ip_address, ip_address, ip_address)
...
def sshScan(ip_address, port):
    ...
    SSHSCAN = "nmap -sV -Pn -p %s --script-args=unsafe=1 --script-args=vulns.showall --script=ssh-auth-methods,ssh-hostkey,ssh-run,sshv1 -oN '../reports/%s/ssh_%s.nmap' %s" % (port, ip_address, ip_address, ip_address)
...
def pop3Scan(ip_address, port):
    ...
    POP3SCAN = "nmap -sV -Pn -p %s --script-args=unsafe=1 --script-args=vulns.showall --script=pop3-brute,pop3-capabilities,pop3-ntlm-info -oN '../reports/%s/pop3_%s.nmap' %s" % (port, ip_address, ip_address, ip_address)
    print bcolors.HEADER + SSHSCAN + bcolors.ENDC
...
def nmapScan(ip_address):
    ...
    for line in lines:
        ...
        if ("tcp" in line) and ("open" in line) and not ("Discovered" in line):
            ...
            port = line.split(" ")[0] # grab the port/ports
            # print port
